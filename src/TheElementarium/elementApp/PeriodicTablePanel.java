...
public class PeriodicTablePanel extends JLayeredPane {
    ...
    public PeriodicTablePanel(ElementGUI frame) {
        ...
        logoLabel = new JLabel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                if (rawLogoImage != null) {
                    Graphics2D g2d = (Graphics2D) g.create();
                    g2d.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
                    g2d.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
                    g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                    g2d.drawImage(rawLogoImage, 0, 0, getWidth(), getHeight(), this);
                    g2d.dispose();
                }
            }
        };
        ...
        try {
            ...
            if (logoURL != null) {
                rawLogoImage = ImageIO.read(logoURL);
            }
        } catch (Exception e) {
            System.err.println("Could not load logo image: " + e.getMessage());
        }
        add(logoLabel, JLayeredPane.DEFAULT_LAYER);
    ...
    private void recalculateResponsiveGrid() {
        ...
        if (logoLabel != null && row7Y > 0) {
            int logoY = row7Y + BLOCK_HEIGHT + 10 + 8;
            int maxAvailableHeight = panelHeight - logoY - 15;
            int threeColumnsWidth = (int)((3 * BLOCK_WIDTH) + (2 * hGap)) + 30;
            int logoSize = Math.min(threeColumnsWidth, maxAvailableHeight);
            if (logoSize < 50) logoSize = 50;
            int logoX = leftMargin + (threeColumnsWidth - logoSize) / 2 - 15;
            if (logoX < leftMargin) logoX = leftMargin;
            if (panelHeight > logoY) {
                logoLabel.setBounds(logoX, logoY, logoSize, logoSize);
                logoLabel.repaint();
            }
        }
