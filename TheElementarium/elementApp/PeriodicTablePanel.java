...
public class PeriodicTablePanel extends JLayeredPane {
    ...
    private final JButton labButton;
    ...
    public PeriodicTablePanel(ElementGUI frame, ActionListener onBackToMenu, ActionListener onOpenLab) {
        this.frame = frame;
        ...
        labButton = new JButton("Elements Laboratory") {
            private boolean isHovered = false;
            private boolean isPressed = false;
            {
                addMouseListener(new MouseAdapter() {
        ...
        labButton.setFont(new Font("Arial", Font.BOLD, 15));
        labButton.setForeground(ElementGUI.DARK_BLUE);
        labButton.setFocusPainted(false);
        labButton.setContentAreaFilled(false);
        labButton.setOpaque(false);
        labButton.setCursor(new Cursor(Cursor.HAND_CURSOR));
        labButton.addActionListener(onOpenLab);
        add(labButton, JLayeredPane.DEFAULT_LAYER);
    ...
    private JButton createElementButton(String symbol) {
        ...
        btn.setBackground(element != null ? element.getGroupColor() : ElementGUI.DEFAULT_PASTEL);
    ...
    private void recalculateResponsiveGrid() {
        ...
        if (labButton != null && row7Y > 0) {
            int buttonY = row7Y + BLOCK_HEIGHT + 15 + 80;
            int buttonWidth = (int)((3 * BLOCK_WIDTH) + (2 * hGap));
            labButton.setBounds(leftMargin - 4, buttonY, buttonWidth + 8, 50);
        }
    ...
    public void filterByGroup(String group) {
        if (group == null || group.isEmpty() || group.equalsIgnoreCase("All Elements")) {
            resetGrid();
            return;
        }
        for (JButton btn : frame.getElementButtons()) {
            Element element = frame.logic.findElement(btn.getText());
            if (element == null) {
                btn.setVisible(true);
            } else {
                btn.setVisible(group.equalsIgnoreCase(element.getGroup()));
            }
        }
        detailPanel.setVisible(false);
        repaint();
